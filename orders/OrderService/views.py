...
class ConfirmAccount(APIView):
    ...
    # Класс для подтверждения адреса

    def post(self, request, *args, **kwargs):  # проверяем обязательные аргументы
...
class AccountDetails(APIView):
    ...
    def get(self, request, *args, **kwargs):  # Возвращаем данные пользователя
    ...
    def post(self, request, *args, **kwargs):  # Изменяем данные пользователя
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Login required'}, status=status.HTTP_403_FORBIDDEN)
...
class ContactView(APIView):
    ...
    def put(self, request, *args, **kwargs):  # Редактируем контакт
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Вы не авторизовались!'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, *args, **kwargs):  # Удаляем указанные контакты
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Вы не авторизовались!'}, status=status.HTTP_403_FORBIDDEN)
...
class PartnerState(APIView):
    ...
    def get(self, request, *args, **kwargs):  # получаем текущий статус заказов у магазина
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Login required'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def post(self, request, *args, **kwargs):  # изменяем текущий статус заказов у магазина
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Log in required'}, status=status.HTTP_403_FORBIDDEN)
...
class BasketView(APIView):
    ...
    def get(self, request, *args, **kwargs):  # получаем содержимое корзины
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Вы не авторизовались!'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def put(self, request, *args, **kwargs):  # Редактируем количество товаров в корзине
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Вы не авторизовались!'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def delete(self, request, *args, **kwargs):  # Удаляем товары из корзины
        if not request.user.is_authenticated:
            return Response({'status': False, 'error': 'Вы не авторизовались!'}, status=status.HTTP_403_FORBIDDEN)
